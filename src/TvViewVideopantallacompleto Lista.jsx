...
export default function TvViewVideo({ 
  ...
  const visibleTracks = hasQueue ? queue.slice(safeIdx, safeIdx + 5) : [EMPTY_TRACK];
...
useEffect(() => {
  ...
  return (
    <div style={{ height: "100vh", background: "#000", overflow: "hidden", position: "fixed", inset: 0, fontFamily: "system-ui, sans-serif" }}>
      ...
      {/* EL VIDEO COMO FONDO DE PANTALLA COMPLETO */}
      <div style={{ position: "fixed", inset: 0, zIndex: 5, background: "#000" }}>
        <div style={{ width: "100%", height: "100%", opacity: (showAd || !track.youtubeId) ? 0 : 1, transition: "opacity 0.3s" }}>
      ...
      {/* INTERFAZ OVERLAY */}
      <div style={{ position: "relative", zIndex: 10, display: "flex", flexDirection: "column", justifyContent: "space-between", height: "100vh", boxSizing: "border-box" }}>

        <div style={{ display: "flex", justifyContent: "flex-end" }}>
          {/* PRÓXIMAS CANCIONES (PANEL SUPERIOR DERECHO) */}
          <div style={{ width: 280, background: "rgba(0,0,0,0.45)", backdropFilter: "blur(12px)", padding: "20px", borderRadius: "0px 0px 0px 24px",  boxShadow: "0 10px 30px rgba(0,0,0,0.5)" }}>
            <div style={{ fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,0.4)", letterSpacing: "0.2em", marginBottom: "1rem" }}>PROXIMAS CANCIONES</div>
            <div style={{ display: "flex", flexDirection: "column" }}>
              {visibleTracks.map((t, i) => {
                const globalIndex = safeIdx + i;
                const active = globalIndex === safeIdx;
                return (
                  <div key={t.id + globalIndex} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.05)", opacity: active ? 1 : 0.55 }}>
                    <div style={{ width: 36, height: 36, borderRadius: 6, flexShrink: 0, overflow: "hidden", border: active ? `1px solid ${t.color}55` : "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
          {/* INFO CANCIÓN ACTUAL (PANEL INFERIOR IZQUIERDO) */}
          <div style={{ flex: 1, maxWidth: "60vw", background: "rgba(0,0,0,0.45)", backdropFilter: "blur(12px)", padding: "10px 20px", borderRadius: "0px 24px 0px 0px",  boxShadow: "0 10px 30px rgba(0,0,0,0.5)" }}>
            <div key={track.id} style={{ animation: "slideUp 0.6s ease" }}>
              <div style={{ fontSize: "1.2rem", fontWeight: 500, color: "#fff", lineHeight: 1.1, letterSpacing: "-0.02em", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{track.title}</div>
          ...
          {/* QR CODE (PANEL INFERIOR DERECHO) */}
          {/* <div style={{ background: "rgba(0,0,0,0.45)", borderRadius: "20px 0px 0px 20px", border: "1px solid rgba(255,255,255,0.08)", padding: "15px", display: "flex", flexDirection: "column", alignItems: "center", gap: "10px", backdropFilter: "blur(12px)", marginLeft: 20 }}> */}
            <div style={{ background: "rgba(0,0,0,0.45)", borderRadius: "20px 0px 0px 20px",  padding: "15px", display: "flex", flexDirection: "column", alignItems: "center", gap: "10px", backdropFilter: "blur(12px)", marginLeft: 20 }}>
